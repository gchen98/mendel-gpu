...
void parse_macs(float sim_len){
  string line;
  ofs_snpinfo<<"CHROM\tPOS\tID\tREF\tALT\tFREQ\n";
  snps = 0;
  int last_basepos = 0;
  while(getline(cin,line)){
    string first = line.substr(0,4);
    if (first.compare("SITE")==0){
      string token,haplo;
      int siteindex;
      float pos,age;
      istringstream iss(line);
      iss>>token>>siteindex>>pos>>age>>haplo;
      // positions are 1-based
      int basepos = static_cast<int>(pos*sim_len);
      ...
      oss_common<<"1\t"<<basepos<<"\tSNP"<<siteindex<<"\t"<<ref_base<<
      "\t"<<alt_base<<"\t"<<get_freq(haplo);
      ofs_snpinfo<<oss_common.str()<<endl;
      if (siteindex % 10000 == 0) cerr<<"site "<<siteindex<<" complete\n";
...
int main(int argc,char * argv[]){
  if (argc<6){
    cout<<"Usage: <fasta_file> <snpinfo_file> <variants_dir> <subjects> <simulated_len>\n";
    return 1;
  }
  srand(1);
  int arg=0;
